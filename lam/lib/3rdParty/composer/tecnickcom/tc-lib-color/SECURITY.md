...
This document describes the security policy for **tc-lib-color**.
...
1. **Email** the maintainer directly at **[info@tecnick.com](mailto:info@tecnick.com)** with the subject line:  
   `[SECURITY] tc-lib-color – <brief description>`
...
- **Environment**: PHP version, OS, library version (output of `composer show tecnickcom/tc-lib-color`).
...
- **Keep dependencies up to date.** Run `composer update` regularly and monitor advisories via [Packagist Security Advisories](https://packagist.org/packages/tecnickcom/tc-lib-color) or tools such as `composer audit`.
...
| GitHub repository | <https://github.com/tecnickcom/tc-lib-color> |
