...
This document describes the security policy for **tc-lib-pdf-encrypt**.
...
1. **Email** the maintainer directly at **[info@tecnick.com](mailto:info@tecnick.com)** with the subject line:  
   `[SECURITY] tc-lib-pdf-encrypt – <brief description>`
...
- **Environment** — PHP version, OS, library version (output of `composer show tecnickcom/tc-lib-pdf-encrypt`).
...
- **Keep dependencies up to date.** Run `composer update` regularly and monitor advisories via [Packagist Security Advisories](https://packagist.org/packages/tecnickcom/tc-lib-pdf-encrypt) or tools such as `composer audit`.
...
| GitHub repository | <https://github.com/tecnickcom/tc-lib-pdf-encrypt> |
